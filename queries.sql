...
FROM vets v
LEFT JOIN specializations sp ON CAST(v.id AS TEXT) = sp.vet_id
LEFT JOIN species s ON CAST(sp.species_id AS INTEGER) = s.id;
...
JOIN animals a ON v.animal_id = a.id
JOIN vets vt ON v.vet_id = vt.id
LEFT JOIN specializations sp ON CAST(vt.id AS TEXT) = sp.vet_id AND CAST(a.species_id AS TEXT) = sp.species_id
WHERE CAST(sp.species_id AS INTEGER) != a.species_id;
